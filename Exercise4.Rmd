...
#Note that in the R Code chunk of Exercise 4.6, just before the "plot the lda results", you do NOT have to change anything in these lines below (as we instructed in the Zoom), as it is now fixed by the above changes

#Note also that when you plot the lda results, you will need to select both lines of code and execute them at the same time for the `lda.arrows()` function to work (as is said in the Instructions - the unnecessary reference to DataCamp can be removed from there, however). So: select and then press Ctrl+Enter (Cmd+Enter) to see the cool plot with the arrows!
